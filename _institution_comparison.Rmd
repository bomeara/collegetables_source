...
Colleges have various expenses. How have they changed through time?

```{r, eval=TRUE}
figure_expenses_string <- ""
try({

	relevant_table <- focal_institutions_table |> subset(`UNITID Unique identification number of the institution` == params$institution_id) |> dplyr::select(`IPEDS Year`, `Total expenses Total amount`, `Academic support Total amount`, `Student service Total amount`, `Research Total amount`, `Instruction Total amount`, `Public service Total amount`, `Auxiliary enterprises Total amount`, `Net grant aid to students Total amount`)
	...
	percentage_table$`Research` <- 100*relevant_table$`Research Total amount`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Instruction` <- 100*relevant_table$`Instruction Total amount`/ relevant_table$`Total expenses Total amount`
	...
	expenses_plot <- ggplot(percentage_tall, aes(x=as.numeric(Year), y=value, group=name, colour=name)) +
		...
		geom_text_repel(data = last_points, aes(label = name), hjust = 0, nudge_x = 0.2, direction = "y", show.legend = FALSE) +
