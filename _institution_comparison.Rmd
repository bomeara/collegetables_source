...
#save(list=ls(), file=paste0("~/Downloads/params_", params$institution_id_in, ".RData"))
		...
		for (row_index in sequence(nrow(final_summary))) {
			try({
				...
				if(nchar(focal_units)>0) {
					...
					significance <- summary(fit)$coefficients[2,4]
...
The sections below show this institution compared with others. The ones listed are ones it has identified as peers, who consider themselves peers, and/or who the federal government considers peers. If a comparison school has the same value as the focal school, its cell is grayed out. Arrows show where there is a signficant trend over time for a school.
...
This has information on the location of the institution. See the [about page](about.html) for more information on what the metrics are and how they are calculated. The goal is to neutrally provide information: for example, some individuals want stringent gun control in an area, some want the opposite: the categories are meant to be descriptive. 
