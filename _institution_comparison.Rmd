...
try({
	if(index_focal_vector['Full-time undergrad enrollment trend']=="Decreasing") {
		notes <- append(notes, "This institution's full-time undergraduate <a href='#enrollment'>enrollment</a> has tended to decrease over time.")
...
if(length(notes)>0) {
	cat("# Notes\nThese are items that bear looking into more closely; they are not necessarily 'good' or 'bad'.\n\n")
	for (note in notes) {
		cat(paste0('* ', note, "\n\n"))	
	}	
}
...
fate_figure_string <- ""
try({
	fate_data <- data.frame(
		year = most_current_info['IPEDS Year'],
		party_short = c("Grad in 4 yr", "Grad in 4.1-6 yr", "Transfer", "Drop out"),
		percentage = c(
			as.numeric(most_current_info['Graduation Rate Bachelors in 4 years or less Grand total']),
			as.numeric(most_current_info['Graduation Rate Bachelors in within 150 percent of normal time Grand total']) - as.numeric(most_current_info['Graduation Rate Bachelors in 4 years or less Grand total']),
			as.numeric(most_current_info['Graduation Rate Transfer out of bachelors Grand total']),
			100 - as.numeric(most_current_info['Graduation Rate Bachelors in within 150 percent of normal time Grand total']) - as.numeric(most_current_info['Graduation Rate Transfer out of bachelors Grand total'])
		),
		colour = c("deepskyblue", "dodgerblue", "khaki3", "red"),
		government = -1+sequence(4)
	)
	fate_data$party_long <- paste0(fate_data$party_short, " (", round(fate_data$percentage,1), "%)")
	if(is.na(fate_data$percentage[3])) {
		fate_data <- fate_data[-4,]	
		fate_data$percentage[3] <- 100 - sum(fate_data$percentage[1:2])
		fate_data$colour[3] <- "red"
		fate_data$party_short[3] <- "Drop out or transfer"
		fate_data$party_long[3] <- paste0(fate_data$party_short[3], " (", formatMe(fate_data$percentage[3],1), "%)")
	}
	scaling <- min(1000, max(300, sqrt(as.numeric(most_current_info['AllUndergrads Grand total']))))
	#scaling <- as.numeric(most_current_info['AllUndergrads Grand total'])
	fate_data$seats <- round(scaling*fate_data$percentage/100)
	fate_data_reformat <- ggparliament::parliament_data(election_data = fate_data,
  		type = "semicircle",
  		parl_rows = floor(scaling/25),
  		party_seats = fate_data$seats
	)
	p_parliament <- ggplot(fate_data_reformat, aes(x, y, colour = party_long)) +
  geom_parliament_seats() + 
  theme_ggparliament() +
  #other aesthetics
  labs(colour = NULL, 
       title = paste0(unname(most_current_info['Institution entity name']), " undergrad outcomes")) +
  scale_colour_manual(values = fate_data_reformat$colour, 
                      limits = fate_data_reformat$party_long) 
	fate_filename_png <- paste0("images/", utils::URLencode(gsub(" ", "", params$institution_id)), "_ug_fate", ".png")

	
	suppressWarnings(suppressMessages({
		ggsave(
			plot = p_parliament,
			filename = paste0('docs/', fate_filename_png),
			bg = "transparent",
			width = params$spark_width*12,
			height= params$spark_height*15,
			units = "px"
		)
	}))
	fate_figure_string <- paste0('<img src=', fate_filename_png, ' alt="Parliament plot showing the outcomes for full time undergraduates">')
	
}, silent=TRUE)
```

`r fate_figure_string`
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, eval=TRUE}
enrollment_data <- focal_focal[,c('IPEDS Year', 'AllUndergrads Grand total')]
colnames(enrollment_data) <- c("Year", "Full time undergrad enrollment")
enrollment_data$Year <- as.numeric(enrollment_data$Year)
enrollment_data$`Full time undergrad enrollment` <- as.numeric(enrollment_data$`Full time undergrad enrollment`)
enrollment_data <- enrollment_data[!is.na(enrollment_data$`Full time undergrad enrollment`),]
if(nrow(enrollment_data)>1) {
	enrollment_filename_png <- paste0("images/", utils::URLencode(gsub(" ", "", params$institution_id)), "_enrollment", ".png")

	p <- ggplot(enrollment_data, aes(x=Year, y=`Full time undergrad enrollment`)) + geom_point() + geom_line() + theme_minimal() + labs(y="Full time undergrad enrollment", x="Year") 
	
	suppressWarnings(suppressMessages({
		ggsave(
			plot = p,
			filename = paste0('docs/', enrollment_filename_png),
			bg = "transparent",
	...
	figure_string <- paste0('<img src=', enrollment_filename_png, ' alt="Line plot showing the the full time undergraduate enrollment over time">')
} else {
	figure_string <- ""
}
```
...
Colleges have various expenses, and the share of those has changed through time. A recent (July 1, 2026) [article](https://www.chronicle.com/article/the-faculty-workload-myth) suggested that faculty salaries tend to be decreasing as a component of all college expenses. This plot (if data are available) shows wages and salaries used for instruction (which include faculty and others supporting instruction), other instructional costs, and other costs as a percentage of a college's overall reported expenses each year.

```{r, eval=TRUE}
figure_expenses_string <- ""
try({

	relevant_table <- focal_institutions_table |> subset(`UNITID Unique identification number of the institution` == params$institution_id) |> dplyr::select(`IPEDS Year`, `Total expenses Total amount`, `Academic support Total amount`, `Academic support Salaries and wages`, `Student service Total amount`, `Research Total amount`, `Research Salaries and wages`, `Instruction Total amount`, `Instruction Salaries and wages`, `Public service Total amount`, `Auxiliary enterprises Total amount`, `Net grant aid to students Total amount`)
	
	percentage_table <- data.frame(Year = relevant_table$`IPEDS Year`)
	percentage_table$`Academic support` <- 100*relevant_table$`Academic support Total amount`/ relevant_table$`Total expenses Total amount`

	percentage_table$`Student services` <- 100*relevant_table$`Student service Total amount`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Research wages` <- 100*relevant_table$`Research Salaries and wages`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Research other` <- 100*(relevant_table$`Research Total amount` - relevant_table$`Research Salaries and wages`)/ relevant_table$`Total expenses Total amount`

	percentage_table$`Instruction wages` <- 100*relevant_table$`Instruction Salaries and wages`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Instruction other` <- 100*(relevant_table$`Instruction Total amount` - relevant_table$`Instruction Salaries and wages`)/ relevant_table$`Total expenses Total amount`
	percentage_table$`Public service` <- 100*relevant_table$`Public service Total amount`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Auxiliary enterprises` <- 100*relevant_table$`Auxiliary enterprises Total amount`/ relevant_table$`Total expenses Total amount`
	percentage_table$`Net grant aid to students` <- 100*relevant_table$`Net grant aid to students Total amount`/ relevant_table$`Total expenses Total amount`
	
	to_cull <- c()
	for (i in sequence(3:ncol(percentage_table))) {
		if(max(percentage_table[,i], na.rm=TRUE)==0) {
			to_cull <- c(to_cull, i)	
		}
	}
	if(length(to_cull)>0) {
		percentage_table <- percentage_table[,-to_cull]	
	}

	percentage_tall <- pivot_longer(percentage_table, cols=!Year)
	percentage_tall <- percentage_tall[!is.na(percentage_tall$value),]
	last_points <- percentage_tall |>
		dplyr::group_by(name) |>
		dplyr::filter(Year == max(Year, na.rm = TRUE)) |>
		dplyr::ungroup()
	
	break_x <- c(range(as.numeric(percentage_tall$Year)), 5+max(as.numeric(last_points$Year)))
	
	expenses_plot <- ggplot(percentage_tall, aes(x=as.numeric(Year), y=value, group=name, colour=name)) +
		geom_line() + theme_minimal() +
		geom_text_repel(data = last_points, aes(label = name), hjust = 0, nudge_x = 0.2, direction = "y", show.legend = FALSE, segment.color = "darkgray") +
		guides(colour='none') + labs(x="Year", y="Percentage", title="Percentage of expenses by source") +  scale_x_continuous(breaks = break_x, labels=c(break_x[1:2]," "), limits=range(break_x)) 
		
	expenses_percentage_filename_png <- paste0("images/", utils::URLencode(gsub(" ", "", params$institution_id)), "_expenses_percentage", ".png")


	suppressWarnings(suppressMessages({
		ggsave(
			plot = expenses_plot,
			filename = paste0('docs/', expenses_percentage_filename_png),
			bg = "transparent",
			width = 2000,
			height= 1000,
			units = "px"
	...
	figure_expenses_string <- paste0('<img src=', expenses_percentage_filename_png, ' alt="Line plot showing expenses by category over time.">')
	if(all(is.na(percentage_tall$value))) {
		figure_expenses_string <- ""
	}
}, silent=TRUE)
```

`r figure_expenses_string`
...
for (row_index in 1:nrow(local_summary)) {
	...
	fn_ecdf <- NULL
	if(length(raw_distribution)>0) {
		fn_ecdf <- stats::ecdf(raw_distribution)
	}
	color_ramp <- color_ramp_neutral
	try({
		if(local_conversion$Better[row_index]=="Higher") {
...
There is a concern that giving changing US demographics, the number of students in the age groups who most commonly attend four year colleges will drop off, decreasing overall enrollment. This is often referred to as the ["demographic cliff"](https://www.chronicle.com/article/will-your-college-survive-the-demographic-cliff?sra=true&cid=gen_sign_in), and it can be a concern for colleges concerned about the risk of falling enrollment. For this section, the analysis uses US census data on the number of people in each state by age, and the proportion of students that come from each state for this particular college, to crudely model what will happen if everything remains constant except the demographic change in the population of 18 year olds in each year -- it does not account for things like the college increasing its admission rate, attracting more students from states without the same demographic changes or from other countries, or changes in the proportion of students who go to college.
...
for (focal_state in unique(population_by_state_by_age$State)) {
	for (focal_age in seq(from=18, to=0, by=-1)) {
		...
		try(count_students <- 0.01 * as.numeric(most_current_info[column_to_find]) * population_by_state_by_age[population_by_state_by_age$State==focal_state & population_by_state_by_age$Age==focal_age, 'Population'])
